...
def calculate_lpips_mh(img_list,
                       img2,
                       input_order='HWC',
                       lpips_net='alex'):
    import torch
    ...
    def _to_tensor(image):
        # image: numpy array in [0,255], shape: HWC or CHW
        if input_order == 'HWC':
            image = image.transpose(2, 0, 1)  # HWC -> CHW
        tensor = torch.from_numpy(image).float() / 127.5 - 1.0  # Normalize to [-1, 1]
        return tensor.unsqueeze(0).to(device)

    img2_tensor = _to_tensor(img2)
    ...
    for key in best_images.keys():
        ...
        img1_tensor = _to_tensor(img1)
        with torch.no_grad():
            best_images_metric[key] = loss_fn(img1_tensor, img2_tensor).item()

    return lpips_scores, np.min(lpips_scores), np.max(lpips_scores), best_images_metric
