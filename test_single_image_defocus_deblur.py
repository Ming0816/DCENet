...
from basicsr.models.archs.DCENetMultiHead_Single_arch import DCENetMHLocal
import cv2
import utils
...
parser = argparse.ArgumentParser(description='Single Image Defocus Deblurring using DCE')

parser.add_argument('--input_dir', default='./datasets/Defocus_Deblurring/test/DPDD/', type=str, help='Directory of validation images')
parser.add_argument('--result_dir', default='./results/Single_Image_Defocus_Deblurring_DCE/', type=str, help='Directory for results')
parser.add_argument('--weights', default='./net_g_Single_Image_DCE.pth', type=str, help='Path to weights')
parser.add_argument('--save_images', action='store_true', help='Save denoised images in result directory')

args = parser.parse_args()

####### Load yaml #######
yaml_file = './options/train/Defocus-Deblur/DefocusDeblur_Single_8bit_DCENet.yml'
import yaml
...
with torch.no_grad():
    for fileI, fileC in tqdm(zip(filesI, filesC), total=len(filesC)):
        ...
        patchI = torch.from_numpy(imgI).unsqueeze(0).permute(0, 3, 1, 2).to(device)
        patchC = torch.from_numpy(imgC).unsqueeze(0).permute(0, 3, 1, 2).to(device)

        restored = model_restoration(patchI)  # [1, N_heads, 3, H, W]
        ...
        for h in range(N_heads):
            ...
            if args.save_images:
                ...
                restored_uint8 = np.uint8((restored_np * 255).round())
                utils.save_img(save_file, restored_uint8)
